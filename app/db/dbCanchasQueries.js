...
const selectTipoCanchas = `SELECT tc.id_tipo_de_cancha AS id, tc.tipo_cancha , tc.duracion_min, tc.duracion_max, tc.ancho, tc.largo, tc.capacidad, JSON_OBJECT('id', s.id_superficie , 'tipo', s.tipo_superficie, 'descripcion', s.descripcion) AS superficie, tc.imagen_url FROM tipos_de_cancha tc LEFT JOIN superficies s ON tc.id_tipo_de_cancha = s.id_superficie`
const selectCanchas = `SELECT can.id_cancha AS id, can.nombre, tc.tipo_cancha, tc.ancho, tc.largo, tc.capacidad, JSON_OBJECT('id', s.id_superficie , 'tipo', s.tipo_superficie, 'descripcion', s.descripcion) AS superficie, can.precio_hora_reserva, can.tiempo_cancelacion, tc.duracion_min, tc.duracion_max, JSON_OBJECT('id', clu.id_club, 'nombre', clu.nombre) AS club, tc.imagen_url FROM canchas can LEFT JOIN tipos_de_cancha tc ON can.id_tipo_de_cancha = tc.id_tipo_de_cancha  LEFT JOIN superficies s ON tc.id_tipo_de_cancha = s.id_superficie LEFT JOIN clubes clu ON can.id_club = clu.id_club`
