...
const selectDisponibilidadRealByFechaAndIdCancha = `SELECT  IF(de.id_disponibilidad IS NULL or de.dia != ? , 'Horario habitual', de.motivo) AS motivo, d.dia_semana, IF(de.id_disponibilidad IS NULL or de.dia != ? , d.hora_inicio, de.hora_inicio) AS hora_inicio, IF(de.id_disponibilidad IS NULL or de.dia != ? , d.hora_fin, de.hora_fin) AS hora_fin, CASE WHEN de.id_cancha IS NULL  and de.dia = ? THEN JSON_OBJECT('id', 'TODAS') ELSE JSON_OBJECT('id', c.id_cancha, 'nombre', c.nombre) END AS cancha, IF(de.cerrado IS NULL or de.dia != ? , 'Abierto', IF(de.cerrado, 'Cerrado', 'Abierto (Horario especial)') ) AS estado, IF(de.ID_disponibilidad IS NULL OR de.dia != ? , ? , de.dia) AS fecha FROM disponibilidad d LEFT JOIN disponibilidad_excepciones de ON DAYOFWEEK(de.dia)-1 = d.dia_semana AND de.dia = ?  LEFT JOIN canchas c ON d.id_cancha = c.id_cancha WHERE ((d.id_cancha = ? ) AND (d.dia_semana = DAYOFWEEK(?)-1))`
