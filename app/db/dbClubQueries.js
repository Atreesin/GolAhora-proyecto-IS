...
//consulta
const selectFullInfoClubes = 'SELECT c.id_club as id, c.nombre, c.CUIT, c.num_telefonico as telefono, c.email, d.calle, d.numero, d.codigo_postal, l.nombre AS localidad, ci.nombre AS ciudad, pr.nombre AS provincia, pa.nombre AS pais FROM clubes c LEFT JOIN direcciones d ON c.id_direccion = d.id_direccion LEFT JOIN localidades l ON d.id_localidad = l.id_localidad LEFT JOIN ciudades ci ON l.id_ciudad = ci.id_ciudad LEFT JOIN provincias pr ON ci.id_provincia = pr.id_provincia LEFT JOIN paises pa ON pr.id_pais = pa.id_pais';
